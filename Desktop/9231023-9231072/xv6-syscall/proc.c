...
int loadProc(){
	...
	/*read( fd,proc->pgdir,sizeof(proc->pgdir));
  	read( fd,&np->sz,sizeof(np->sz) );
  	write( fd,np->parent,sizeof(np->parent) );
  	write( fd,np->tf,sizeof(np->tf) );
	write( fd,np->cwd,sizeof(np->cwd) );
 	write( fd,np->name,sizeof(np->name) );*/
	//pid = np->pid;
	//np->tf->eax = 0;
	//np->cwd = idup(np->cwd)
  	//acquire(&ptable.lock);
...
int saveProc(){
	//int fd = open("HProcess.txt",O_WRONLY|O_CREATE); 
